...
function mostrarCargos() {
    ...
    if (cidade === "JARU") {
        cargos = ["Vendedor", "Auxiliar de Limpeza", "Montador", "Motorista", "Crediarista", "Gerente de Vendas", "Auxiliar Centro de Distribuição", "Contabilidade", "Compras", "Financeiro", "Marketing", "Recursos Humanos", "Análise de Crédito", "Cobrança", "Tecnologia da Informação"];
    } else {
...
document.getElementById('curriculoForm').addEventListener('submit', function(event) {
    ...
    .then(data => {
        ...
        successMessage.innerHTML = `<h2>${data.message || 'Você enviou o seu currículo com sucesso!'}</h2><h3><p>Aguarde, nossa equipe irá analisar seu currículo!<br> Caso você já tenha enviado um currículo com o mesmo CPF anteriormente, ele será atualizado com essas informações</h3>`;
    })
    .catch(error => {
